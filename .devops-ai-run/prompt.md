...
env: "dev"
target_branch: "dev"
application_name: "ai-demogaanaai-devops"
platform: "eks"
```
...
- application name: ai-demogaanaai-devops
...
- GitHub Actions role ARN created by EC2 agent: arn:aws:iam::478546323821:role/ai-demogaanaai-devops-dev-gha
- GitHub Actions attached policy ARN: arn:aws:iam::478546323821:policy/ai-demogaanaai-devops-dev-gha-eks
- Terraform state S3 bucket created by EC2 agent: devops-ai-tfstate-478546323821
- Terraform state key for this environment: ai-demogaanaai-devops/dev/terraform.tfstate
...
- infra/terraform/ files already exist in this repo (from the file scan above): true
...
Operator-maintained infra sizing config for application 'ai-demogaanaai-devops' (from infra-config.yaml on the EC2 host - plain non-secret Terraform defaults, not Secrets Manager-backed, not IAM-scoped):
...
VPC name: `ai-demogaanaai-devops` by itself (no env suffix) - a VPC's name is
...
- IAM naming is a permissions boundary: every IAM role and customer-managed
  ...
  must use the prefix `ai-demogaanaai-devops-dev-`. Explicitly set module
  role/policy names; never accept defaults such as `default-eks-node-group-*`
  or `AmazonEKS_EBS_CSI_Policy-*`. For IAM modules that create both a role and
  a customer-managed policy, setting `name` controls only the role: also set
  the supported `policy_name` input explicitly. Never allow shared defaults
  such as `EBS_CSI` or `VPC_CNI`, because IAM policy names are unique
  account-wide and another application will fail with `EntityAlreadyExists`.
- EBS CSI IAM module compatibility is mandatory. When using
  `terraform-aws-modules/iam/aws//modules/iam-role-for-service-accounts`, pin
  a compatible module version and configure the role with `name`,
  `policy_name`, and `use_name_prefix = false`, plus
  `attach_ebs_csi_policy = true`. Do not emit
  the legacy/unsupported arguments `role_name` or `role_name_use_prefix` for
  that module. If an older `iam-role-for-service-accounts-eks` submodule is
  already present during an update, preserve its pinned version and use only
  inputs declared by that exact version; do not mix input names from the two
  module interfaces. A new configuration should follow this shape:
  ```hcl
  module "ebs_csi_irsa" {
    source  = "terraform-aws-modules/iam/aws//modules/iam-role-for-service-accounts"
    version = "<pin a compatible current major/minor>"
    ...
    # Configure oidc_providers using the schema supported by the pinned version.
  }
  ```
  Apply the same rule to VPC CNI IRSA/Pod Identity modules: set explicit
  app/environment-scoped `name` and `policy_name` values, for example
  `ai-demogaanaai-devops-dev-vpc-cni`. For
  `iam-role-for-service-accounts` v6.x on an IPv4 cluster, also set
  `vpc_cni_enable_ipv4 = true` with `attach_vpc_cni_policy = true`; otherwise
  the generated policy can contain only `ec2:CreateTags`, leaving out the ENI
  and private-IP actions required by `aws-node`. Enable IPv6 only when the
  cluster is actually configured for IPv6. Confirm these inputs against the
  exact pinned module version, inspect the rendered policy for the required
  EC2 network-interface/IP actions, and run Terraform validation.
- Apply `Application = "ai-demogaanaai-devops"` and `Environment = "dev"`
...
- Node-join reliability is mandatory for fresh clusters:
  - Put managed nodes in the private subnets and reference `module.vpc.vpc_id`
    and `module.vpc.private_subnets` normally. Never add module-level
    `depends_on = [module.vpc]` to `module "eks"`: broad module dependencies
    defer the EKS module's `aws_partition`/`aws_caller_identity` data until
    apply, which makes the nested managed-node-group `count` unknown and causes
    `terraform plan` to fail with `Invalid count argument`.
  - For a fresh deployment, sequence VPC readiness in `infra-apply.yml`, not in
    the EKS module graph: first run an idempotent targeted apply for the root VPC
    module (`terraform apply -auto-approve -target=module.vpc`), verify NAT and
    private default routes are active, and only then run the normal full
    `terraform apply -auto-approve`. This remains one push-triggered apply job;
    PRs still run one complete, untargeted Terraform plan. Skip the targeted
    phase on updates when real state already contains the VPC.
  - Enable the private cluster endpoint. If the public endpoint is also needed
    by GitHub-hosted runners, keep it enabled but do not open worker-node
    ingress to `0.0.0.0/0`. Use the EKS module's cluster/node security groups
    and its scoped cluster-to-node and node-to-cluster rules.
  - Preserve the EKS module's standard managed-node IAM policies and bootstrap
    user data. Never replace/merge AL2023 `nodeadm` user data unless the ticket
    explicitly requires a launch template and the generated MIME/node config
    has been validated. Ensure the node role is represented by the module's
    EC2_LINUX access entry/auth mapping before considering the node group ready.
  - Set the managed node-group Terraform timeouts explicitly (`create = "60m"`,
    `update = "60m"`, `delete = "30m"`) using inputs supported by the pinned
    EKS module version. These Terraform timeouts do not override AWS's own node
    health deadline; they prevent the client from abandoning a healthy but slow
    AWS operation prematurely.
    ```hcl
    eks_managed_node_groups = {
      default = {
        # name, instance_types, scaling and IAM settings omitted here
        timeouts = {
          create = "60m"
          update = "60m"
          delete = "30m"
    ...
    ```
- Before finishing generated Terraform/workflows, verify every EKS bootstrap
  prerequisite explicitly; successful EC2 creation alone does not mean a node
  can join Kubernetes:
  - The node instance role must trust `ec2.amazonaws.com` and include
    `AmazonEKSWorkerNodePolicy` plus ECR pull permissions equivalent to the
    current AWS-managed `AmazonEC2ContainerRegistryPullOnly` policy.
    `AmazonEKS_CNI_Policy` must also be available either on the node role or,
    preferably, through dedicated IRSA/Pod Identity for the `aws-node` service
    account before CNI readiness is required. Never use the GitHub Actions
    deployment role as the EC2 node instance role.
  - With EKS access-entry authentication, require an `EC2_LINUX` entry for the
    exact pathless node-role ARN and standard node username/groups. With legacy
    authentication, require the equivalent unmodified `aws-auth` mapping. Keep
    the cluster authentication mode compatible with the pinned module.
  - Every private node subnet must have an active `0.0.0.0/0` route through a
    working NAT gateway, or the complete VPC endpoints required by a
    no-internet cluster (including ECR API/DKR, S3, STS and EKS/API access).
    Confirm that regional AWS STS is enabled.
  - Enable VPC DNS support/hostnames and use DHCP options providing the regional
    domain name plus `AmazonProvidedDNS`, so kubelet gets the expected private
    DNS node name.
  - Node and cluster security groups must allow node egress to the cluster API
    on TCP 443 and required control-plane-to-node traffic. Keep rules scoped by
    security-group ID; never open unrestricted node ingress from `0.0.0.0/0`.
  - Use an EKS-optimized AMI compatible with the Kubernetes version and CPU
    architecture. For custom launch templates, validate cluster name, endpoint,
    certificate authority, service CIDR and AL2023 `nodeadm` content against
    the actual EKS module outputs.
  - Private endpoint access must be enabled for private nodes, or public
    endpoint CIDRs must include their real NAT egress address.
...
- Built-in defaults (used only if infra-config and the ticket don't override):
  ...
  spread across 3 AZs within that single region), an Internet Gateway and
  public `0.0.0.0/0` routes, and NAT egress for every private subnet. Default
  to one cost-saving NAT gateway with an Elastic IP (`enable_nat_gateway =
  true`, `single_nat_gateway = true`); use one NAT per AZ only when the ticket
  or infra-config requests high availability. Every private route table must
  have `0.0.0.0/0` pointing to an `available` NAT gateway before EKS/node-group
  creation. Also apply the standard public `kubernetes.io/role/elb = 1` and
  private `kubernetes.io/role/internal-elb = 1` subnet tags. Node defaults:
  node_instance_types = ["t3.medium"] with desired_size = 4 / min_size = 2 /
  max_size = 6. ECR repository name: `ai-demogaanaai-devops-dev`.
- ECR repository created whenever this platform's infra is requested - one
  ...
  a repo. Enable immutable tags, encryption, and ECR scan-on-push. Follow the
  common pre-push Trivy and post-push ECR scan gates; deploy by image digest
...
- Default add-ons, always included when EKS infra is created, whether or not
  ...
  - With `terraform-aws-modules/eks/aws`, configure the `vpc-cni` add-on with
    `before_compute = true`. Never defer VPC CNI creation until after managed
    node groups: nodes can authenticate and register but remain `NotReady`
    without the CNI DaemonSet, while Terraform waits for the node group before
    creating the add-on, producing a node-readiness dependency deadlock and
    eventual `NodeCreationFailure`. During validation, explicitly confirm that
    the plan orders `aws_eks_addon` for `vpc-cni` before the managed node group.
...
EKS infra-apply health gate and one-time recovery are mandatory:

- Do not let `set -e` hide the first Terraform failure. Capture the exit code
  and print `aws eks describe-nodegroup` status/health issues for every managed
  node group belonging to this cluster.
- Never cache or select the first lexicographic `default-*` node-group name.
  Generated names change after replacement and AWS can retain older failed or
  deleting groups. After each apply, rediscover all managed node groups,
  recover only a `CREATE_FAILED`/`NodeCreationFailure` group whose name appears
  in that apply's captured Terraform output, and use an `ACTIVE` group with no
  health issues for the Kubernetes readiness gate.
- When the first apply fails specifically because a managed node group is
  `CREATE_FAILED` with `NodeCreationFailure`, automatically run exactly one
  targeted Terraform replacement for the node-group resource address that is
  actually present in `terraform state list`, then stop retrying. Never use a
  permanently unconditional `-replace`, and never retry other Terraform/AWS
  failures as though they were node bootstrap failures.
- After apply/recovery, require the EKS node group to report `ACTIVE` with no
  health issues. Update kubeconfig with the workflow's OIDC identity and wait
  up to 15 minutes for all expected nodes to exist and become Kubernetes
  `Ready`; print `kubectl get nodes -o wide`, node-group health, and relevant
  events on failure. Application build/deploy must not begin until this gate
  succeeds.
- On `NodeCreationFailure`, print `describe-nodegroup` health issues/resource
  IDs, Auto Scaling instance health, and instance subnet, route, security-group
  and IAM-profile metadata, plus available EC2 console output and EKS
  authenticator/audit evidence. Recommend the read-only
  `AWSSupport-TroubleshootEKSWorkerNode` runbook or node-local
  `nodeadm`/`kubelet`/`containerd`/CNI journals for deeper diagnosis. Never
  claim that Terraform state alone identifies the bootstrap root cause.
- Before the one allowed replacement retry, list the node subnet route tables
  and NAT gateway state without printing credentials. If any private subnet
  lacks an active NAT/default route (and this is not intentionally a complete
  private-endpoint design), do not retry the node group: fail with the network
  prerequisite as the blocking cause instead of masking it as a timeout.
- This guarded recovery belongs in the normal reusable `infra-apply.yml` flow;
  do not rely on `workflow_dispatch` existing only on a non-default branch.
  A second `NodeCreationFailure` is blocking and must retain diagnostics rather
  than loop, open security groups publicly, or continue application deployment.
...
the wait. Keep the rollout-status check after the workload apply.
...
2. Every other named resource that AWS enforces name-uniqueness on within an account+region (clusters, services, IAM roles, buckets, distributions, etc.): name it `ai-demogaanaai-devops-dev` unless the platform section above specifies a different convention for that resource type.
...
10. Container image security reporting is mandatory whenever any workflow builds or publishes a Docker/OCI image (EKS, ECS, or an explicitly containerized EC2 deployment), but is advisory for now and must not block the workflow:
   - Build one local image tagged with the immutable commit SHA. Never deploy or scan only `latest`.
   - Before any `docker push`, run Trivy through Docker against that exact local image by mounting `/var/run/docker.sock` and a dedicated `${{ github.workspace }}/trivy-report` output directory. The Trivy container must be pinned by an operator-reviewed immutable `aquasec/trivy:0.53.0` reference; never use `latest`, a floating version tag, or an unpinned scanner action.
   - Run the pre-push scan in two non-blocking steps: first generate `trivy-report/trivy-report.html` with `--format template --template "@contrib/html.tpl" --severity HIGH,CRITICAL --no-progress --exit-code 0`; then run a table-format summary against the same image with `--severity HIGH,CRITICAL --no-progress --exit-code 0`. Set `continue-on-error: true` on each Trivy step so scanner runtime errors are advisory too. Keep the table and summary visible in the job log. The push and all later workflow steps must proceed regardless of the scan result.
   - Also generate a machine-readable JSON or SARIF report when supported. Upload the complete `trivy-report` directory with `actions/upload-artifact@v4`, artifact name `trivy-security-report-${{ github.run_id }}`, `if: always()`, `if-no-files-found: error`, and a finite retention period. Artifact upload must execute even when the scan reports findings.
   ...
   - After pushing, resolve the pushed image digest, wait for the ECR scan to complete with a bounded timeout, call `aws ecr describe-image-scan-findings`, and print the scan status plus the count for every returned severity in the pipeline log (for example, `CRITICAL: 1`, `HIGH: 4`, `MEDIUM: N`, `LOW: N`). Do not print credentials or full environment data. Set `continue-on-error: true` on every ECR scan polling/reporting step: record all findings, but always continue deployment to EKS/ECS/EC2 with the digest-qualified image.
   - Retain/upload scanner reports as workflow artifacts when practical, but never hide the severity summary from the job log. Vulnerability findings at any severity, scan timeouts, unsupported scan configuration, missing results, and scanner execution errors must be clearly reported but must not block the workflow.
...
5. Use this shape:
   ...
   terraform:
     variables:
       vpc_id:
         secret_id: /apps/dev/ai-demogaanaai-devops/terraform
         json_key: vpc_id
         tf_var: vpc_id
   runtime:
     env:
       DATABASE_URL:
         secret_id: /apps/dev/ai-demogaanaai-devops/runtime
